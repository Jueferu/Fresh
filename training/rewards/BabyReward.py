...
class BabyReward(RewardFunction):
    def __init__(self):
        ...
        self.event_reward = EventReward(touch=1)
    ...
    def reset(self, initial_state: GameState):
        ...
        self.event_reward.reset(initial_state)

    def get_reward(self, player: PlayerData, state: GameState, previous_action) -> float:
        in_air_reward = self.in_air_reward.get_reward(player, state, previous_action) * .15
        speed_toward_ball_reward = self.speed_toward_ball_reward.get_reward(player, state, previous_action) * 5
        ...
        return in_air_reward + speed_toward_ball_reward + face_toward_ball_reward + event_reward